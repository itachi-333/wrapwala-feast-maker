      ...
      <div className="max-w-7xl mx-auto px-4 grid lg:grid-cols-2 gap-16 items-center" ref={ref}>
        ...
        <div className="reveal-item relative grid grid-cols-2 gap-4" data-delay="0">
          ...
          <div className="absolute -bottom-8 -right-4 w-24 h-24 rotate-stamp">
            <div className="w-full h-full rounded-full border-2 border-dashed border-rust flex items-center justify-center bg-warm-cream/90 backdrop-blur-sm shadow-lg">
        ...
        {/* Right text */}
        <div>
          <span className="reveal-item text-rust font-script text-lg" data-delay="50">Who We Are</span>
          <h2 className="reveal-item text-4xl md:text-5xl font-display font-bold text-deep-brown mb-6 mt-2" data-delay="100">
            Our <span className="font-script text-rust">Story</span>
          </h2>
          <p className="reveal-item text-deep-brown/60 mb-4 leading-relaxed" data-delay="200">
            WrapWala was born from late-night memories of Delhi's bustling street stalls and Kolkata's legendary kathi rolls. We brought those bold, unapologetic flavours to the streets of London.
          </p>
          <p className="reveal-item text-deep-brown/60 mb-8 leading-relaxed" data-delay="300">
          ...
          <div className="grid grid-cols-2 gap-4">
            {pillars.map((p, i) => (
              <div
                key={i}
                className="reveal-item bg-white rounded-xl p-4 hover:shadow-md transition-all border border-rust/10"
                data-delay={`${300 + i * 100}`}
              >
                <span className="text-2xl">{p.icon}</span>
                <h4 className="font-display font-bold text-deep-brown mt-2 text-sm">{p.title}</h4>
                <p className="text-xs text-deep-brown/50 mt-1">{p.desc}</p>
              </div>
            ))}
          </div>
        </div>
